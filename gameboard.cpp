...
bool GameBoard::fill_randomly(unsigned int seed, unsigned int size)
{
    ...
    return fill_from_input('"' + input + '"', size);
}

bool GameBoard::fill_from_input(const std::string &input, unsigned int size)
{
    recreate_board(size);
...
std::vector<std::vector<Element_type>> GameBoard::get_board() const
{
    return board_;
}

void GameBoard::recreate_board(unsigned int size)
{
    board_.clear();
    std::vector<Element_type> row(size, EMPTY);
    for(unsigned int i = 0; i < size; ++i)
